...
if (! function_exists('normalize_business_enabled_modules')) {
    /**
     * business.enabled_modules is cast to array on the model but may be a JSON string when read from
     * session dot notation or legacy array session data. PHP 8+ requires an array for in_array().
     *
     * @param  mixed  $value
     */
    function normalize_business_enabled_modules($value): array
    {
        if (is_array($value)) {
            return $value;
        }
...
if (! function_exists('session_business_enabled_modules')) {
    /**
     * Enabled core POS modules for the current session's business (always a plain array).
     */
    function session_business_enabled_modules(): array
    {
        ...
        $b = session('business');
        if ($b instanceof \App\Business) {
            return normalize_business_enabled_modules($b->enabled_modules);
        }
        if (is_array($b) && array_key_exists('enabled_modules', $b)) {
            return normalize_business_enabled_modules($b['enabled_modules']);
        }
