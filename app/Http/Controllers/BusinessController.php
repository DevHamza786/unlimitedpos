...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        if (empty($business_id) && auth()->check() && ! empty(auth()->user()->business_id)) {
            $business_id = auth()->user()->business_id;
            $user_session = request()->session()->get('user', []);
            if (is_array($user_session)) {
                $user_session['business_id'] = $business_id;
        ...
        if ($business === null) {
            return redirect()->action([HomeController::class, 'index'])->with('status', [
                'success' => 0,
                'msg' => 'No business found for this login. If you seeded demo data, use username admin / password 123456, then log out and log in again. Otherwise run: php artisan db:seed --class=FullDummyWithTestAdminSeeder',
            ]);
        }
        ...
        $woocommerce_locations = BusinessLocation::forDropdown($business_id);

        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types', 'woocommerce_locations'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business->woocommerce_enabled = $request->boolean('woocommerce_enabled');

            $wooUrl = trim((string) $request->input('woocommerce_store_url', ''));
            $business->woocommerce_store_url = $wooUrl !== '' ? rtrim($wooUrl, '/') : null;

            if ($request->filled('woocommerce_consumer_key')) {
                $business->woocommerce_consumer_key = trim($request->input('woocommerce_consumer_key'));
            }
            if ($request->filled('woocommerce_consumer_secret')) {
                $business->woocommerce_consumer_secret = trim($request->input('woocommerce_consumer_secret'));
            }

            $business->woocommerce_import_orders_enabled = $request->boolean('woocommerce_import_orders_enabled');
            if ($business->woocommerce_import_orders_enabled) {
                if (empty($business->woocommerce_webhook_token) || $request->boolean('woocommerce_regenerate_webhook_token')) {
                    $business->woocommerce_webhook_token = \Illuminate\Support\Str::random(48);
                }
            }

            $wooLoc = $request->input('woocommerce_default_location_id');
            if ($wooLoc !== null && $wooLoc !== '') {
                $lid = (int) $wooLoc;
                $business->woocommerce_default_location_id = BusinessLocation::where('business_id', $business->id)
                    ->where('id', $lid)
                    ->exists() ? $lid : null;
            } else {
    ...
    /**
     * Test WooCommerce REST credentials (core integration; no addon module required).
     */
    public function postTestWooCommerce(Request $request, WooCommerceConnectionService $wooCommerce): JsonResponse
    {
        if (! auth()->user()->can('business_settings.access')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($key === '' || $secret === '' || $url === '') {
            ...
            if ($business !== null) {
                ...
                if ($key === '' && ! empty($business->woocommerce_consumer_key)) {
                    $key = $business->woocommerce_consumer_key;
                }
                if ($secret === '' && ! empty($business->woocommerce_consumer_secret)) {
                    $secret = $business->woocommerce_consumer_secret;
                }
            }
        }
