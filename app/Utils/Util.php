        ...
        if (! empty($location)) {
            ...
            if ($location !== null) {
                ...
                if ($business !== null && ! empty($business->custom_labels)) {
                    $decoded = json_decode($business->custom_labels, true);
                    $custom_labels = is_array($decoded) ? $decoded : [];
                }
            }
    ...
    public function allModulesEnabled($business_id = null)
    {
        ...
        if (session()->has('business')) {
            ...
            if ($b instanceof Business) {
                ...
                $enabled_modules = $b['enabled_modules'] ?? null;
            }
        }

        if ($enabled_modules === null && ! empty($business_id)) {
            $biz = Business::find($business_id);
            $enabled_modules = $biz ? $biz->enabled_modules : null;
        }

        return normalize_business_enabled_modules($enabled_modules);
