...
@section('content')
            ...
            <div class="form-group has-feedback {{ $errors->has('username') ? ' has-error' : '' }}">
                @php
                    ...
                    if(config('app.env') == 'demo'){
                        ...
                        $module_statuses = [];
                        if (is_readable(base_path('modules_statuses.json'))) {
                            $module_statuses = json_decode(file_get_contents(base_path('modules_statuses.json')), true) ?: [];
                        }
                        $woocommerce_module_on = ! empty($module_statuses['Woocommerce']);
                        ...
                        if ($woocommerce_module_on) {
                            $demo_types['woocommerce'] = 'woocommerce_user';
                        }
    ...
    <div class="col-md-12 col-xs-12" style="padding-bottom: 30px;">
        @component('components.widget', ['class' => 'box-primary', 'header' => '<h4 class="text-center">Demo Shops <small><i> Demos are for example purpose only, this application <u>can be used in many other similar businesses.</u></i></small></h4>'])
            ...
            @if(!empty($woocommerce_module_on))
